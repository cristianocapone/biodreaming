...
class AGEMONE:
    ...
    def step(self, state : np.ndarray, action : Optional[np.ndarray] = None, deterministic : bool = False, **kwd) -> Any:
    ...
    def reset(self, spikes : Optional[np.ndarray] = None):
    ...
    def forget(self, J_rec : Optional[np.ndarray] = None, J_out  : Optional[np.ndarray] = None):
...
class Actor(AGEMONE):
    ...
    def policy(self, state : np.ndarray, mode : Optional[str] = None):
...
class Planner(AGEMONE):
    ...
    def reset(self, spikes : Optional[np.ndarray] = None) -> None:
        super().reset(spikes)
